  ...
  for (int i = 0; i < placements.size(); i++) {
    for (int j = 0; j < placements.size(); j++) {
      if (i != j) {
        ...
        if (newPos.first == pos1.first && newPos.second == pos1.second) {
          newPos = {pos1.first + diff.first, pos2.first + diff.first};
        }
        if (newPos.first >= 0 && newPos.first < width && newPos.second >= 0 &&
